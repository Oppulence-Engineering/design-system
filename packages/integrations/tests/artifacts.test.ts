...
test("the built root entry resolves every documented runtime export", async () => {
  ...
  expect(typeof server.createIntegrationConnectionLinkRuntime).toBe("function");
  expect(typeof server.createIntegrationConnectionLinkRoutes).toBe("function");
  ...
  expect(typeof server.createBuiltInIntegrationApiKeyRuntime).toBe("function");
