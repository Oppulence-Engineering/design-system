 ...
 * triggers.
 */
const EXECUTABLE_PROVIDERS = 138;
const EXECUTABLE_ACTIONS = 2423;
...
describe("provider parity coverage gate", () => {
  ...
  test("declared pack coverage matches what the registry executes", () => {
    ...
    expect(report.providers).toBe(115);
    ...
    expect(report.deferredOperations).toBe(151);
    ...
    expect(report.byLane.typed_rest.operations).toBe(743);
  ...
  test("every typed REST action records the SDK review that allows it", () => {
    ...
    expect(restActions).toHaveLength(743);
  ...
  test("reports the work remaining against the pinned source", () => {
    ...
    expect({
      ...
      providersRemaining: 94,
      actionsRemaining: 1467,
