...
export type SqlConnectionFactory = (credential: {
...
export interface SqlPackOptions {
  connect?: SqlConnectionFactory;
}
...
export function createMySqlPack(
  options: SqlPackOptions = {},
): IntegrationProviderPack {
  return createProtocolPack<SqlConnection>({
    integrationId: "mysql",
    driver: "mysql2",
    operations: sqlOperations("mysql", "mysql"),
    connect: options.connect ?? connectMysql,
  });
}
