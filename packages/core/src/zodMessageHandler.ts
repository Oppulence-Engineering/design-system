...
export type ZodMessageValueSchema<
...
export type ZodMessageCatalogSchema = {
  // biome-ignore lint/suspicious/noExplicitAny: required for Zod discriminated union generic inference
...
export type ZodMessageHandlers<TCatalogSchema extends ZodMessageCatalogSchema> =
  Partial<{
    [K in keyof TCatalogSchema]: (
      payload: z.infer<TCatalogSchema[K]>,
    ) => Promise<unknown>;
  }>;

export type ZodMessageHandlerOptions<
...
> {
  ...
  readonly #handlers: ZodMessageHandlers<TMessageCatalog> | undefined;
  readonly #logger: StructuredLogger | Console;
  ...
  > {
    ...
    const ack = await handler(
      parsedMessage.data.payload as z.output<
        TMessageCatalog[keyof TMessageCatalog]
      >,
    );
    ...
    for (const eventName of Object.keys(this.#schema)) {
      emitter.on(
        ...
        async (message: unknown, callback?: unknown): Promise<void> => {
          ...
          let ack: Awaited<
            ReturnType<ZodMessageHandler<TMessageCatalog>["handleMessage"]>
          >;

          const isMessageObject =
            message !== null && typeof message === "object";

          if (isMessageObject && "payload" in message) {
            ack = await this.handleMessage({ ...message, type: eventName });
          } else if (isMessageObject) {
            // Handle messages not sent by ZodMessageSender
            const { version, ...payload } = message as Record<string, unknown>;
            ...
            // Handle primitive payloads gracefully
            ack = await this.handleMessage({
              type: eventName,
              payload: message,
            });
...
export function parseMessageFromCatalog<
  TMessageCatalog extends ZodMessageCatalogSchema,
>(
  message: unknown,
  schema: TMessageCatalog,
...
type ZodMessageSenderCallback<TMessageCatalog extends ZodMessageCatalogSchema> =
  (message: {
...
export type ZodMessageSenderOptions<
  TMessageCatalog extends ZodMessageCatalogSchema,
> = {
  schema: TMessageCatalog;
...
export class ZodMessageSender<TMessageCatalog extends ZodMessageCatalogSchema> {
  readonly #schema: TMessageCatalog;
  readonly #sender: ZodMessageSenderCallback<TMessageCatalog>;
  readonly #canSendMessage?: ZodMessageSenderOptions<TMessageCatalog>["canSendMessage"];
  ...
  public async send<K extends keyof TMessageCatalog>(
    ...
    try {
      await this.#sender({
        type,
        payload: parsedPayload.data as z.output<
          TMessageCatalog[keyof TMessageCatalog]
        >,
        version: "v1",
      });
...
export async function sendMessageInCatalog<
  TMessageCatalog extends ZodMessageCatalogSchema,
>(
...
export type MessageCatalogToSocketIoEvents<
  TCatalog extends ZodMessageCatalogSchema,
> = {
