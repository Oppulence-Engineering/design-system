...
import type { RequireKeys } from "../types/index.ts";
import {
  MachineConfig,
  MachinePreset,
  MachinePresetName,
...
export type ProdTaskRunExecutionPayload = z.infer<
  typeof ProdTaskRunExecutionPayload
>;
...
export const QueueManifest = z.object({
  ...
  concurrencyLimit: z.number().int().min(0).max(100_000).optional().nullable(),
...
const RegexSchema = z.custom<RegExp>((val: unknown) => {
  try {
...
export type TaskRunExecutionLazyAttemptPayload = z.infer<
  typeof TaskRunExecutionLazyAttemptPayload
>;
...
export const RunChainState = z.object({
  concurrency: z
    .object({
      queues: z.array(
        z.object({ id: z.string(), name: z.string(), holding: z.number() }),
      ),
