...
export function isSchemaValibotEsque<TInput, TParsedInput>(
  ...
  return (
    ...
    "_types" in ((schema as Record<string, unknown>).schema as object)
  );
}
...
export function isSchemaArkTypeEsque<TInput, TParsedInput>(
  ...
  return typeof schema === "object" && "inferIn" in schema && "infer" in schema;
}
...
// biome-ignore lint/suspicious/noExplicitAny: required for generic schema inference across multiple schema libraries
type _Schema = SchemaWithInputOutput<any, any> | SchemaWithoutInput<any>;
...
// biome-ignore lint/suspicious/noExplicitAny: required for generic schema type erasure
export type AnySchemaParseFn = SchemaParseFn<any>;

export function getSchemaParseFn<TType>(
  procedureParser: Schema,
): SchemaParseFn<TType> {
  // biome-ignore lint/suspicious/noExplicitAny: polymorphic schema parser detection requires dynamic property access
