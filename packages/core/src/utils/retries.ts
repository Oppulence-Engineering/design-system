import { calculateResetAt as calculateResetAtInternal } from "../retry.ts";
import type { FetchRetryOptions, RetryOptions } from "../schemas";
...
export function calculateNextRetryDelay(
  ...
  const timeout = Math.min(
    maxTimeoutInMs,
    random * minTimeoutInMs * factor ** (attempt - 1),
  );
