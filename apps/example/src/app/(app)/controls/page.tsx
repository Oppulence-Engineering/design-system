...
function ControlCard({
  ...
  category: (typeof controlCategories)[0];
}) {
...
export default function ControlsPage() {
  ...
  const passedControls = controlCategories.reduce(
    (sum, c) => sum + c.passed,
    0,
  );
  ...
  return (
    <PageLayout padding="none" container={false}>
      <PageHeader title="Controls">
        <PageHeaderDescription>
          Monitor and manage your security controls across {totalControls}{" "}
          requirements.
        </PageHeaderDescription>
        <PageHeaderActions>
          <Button variant="outline" iconLeft={<FilterIcon />}>
            Filter
          </Button>
        </PageHeaderActions>
      </PageHeader>
      ...
      <Stack gap="4">
        <HStack gap="4">
          ...
          <Badge variant="secondary">
            {Math.round((passedControls / totalControls) * 100)}% complete
          </Badge>
        </HStack>
